...
const DoctorsList = () => {
  ...
  useEffect(() => {
    async function fetchDoctorsList() {
      try {
        ...
        setDoctors(data); // Set the fetched data in the component's state
      } catch (error) {
  ...
  // Call the asynchronous function to fetch the list of doctors
  ...
  return (
    <div>
      ...
      <ul role="list" class="divide-y divide-gray-100">
        {
          doctors.map(doctor=><DoctorProfile id={doctor.id} name={doctor.name}/>)
        }
      </ul> 
    </div>
  )
...
<div class="hidden shrink-0 sm:flex sm:flex-col sm:items-end">
 <button className='m-1 p-2 rounded bg-[#38bdf8]'>View Details</button>
</div>
</li>
